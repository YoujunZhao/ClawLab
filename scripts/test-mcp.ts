...
async function main() {
	const distServerScript = resolve(PROJECT_ROOT, 'mcp-server', 'dist', 'index.js')
	const sourceServerScript = resolve(PROJECT_ROOT, 'mcp-server', 'src', 'index.ts')
	const serverScript = existsSync(distServerScript) ? distServerScript : sourceServerScript
	const srcRoot = resolve(PROJECT_ROOT, 'src')
	const runtimeCommand = process.env.MCP_TEST_RUNTIME || process.execPath || 'node'

	section('1. Spawning MCP server (stdio transport)')
	console.log(`  Server: ${serverScript}`)
	console.log(`  SRC_ROOT: ${srcRoot}`)
	console.log(`  Runtime: ${runtimeCommand}`)
	...
	try {
		...
		if (resourcesResult.resources.length > 10) {
			console.log(`    …and ${resourcesResult.resources.length - 10} more`)
		}
